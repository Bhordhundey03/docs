...
<CardGroup cols={2}>
  <Card title="Create a bulk transfer" icon="upload" href="/api-reference/bulk-transfers/create">
    Submit a batch of transfers as JSON.
  </Card>
  <Card title="Upload via Excel" icon="file-spreadsheet" href="/api-reference/bulk-transfers/upload">
    Submit a batch by uploading a spreadsheet instead.
  </Card>
  ...
  <Card title="Delete" icon="trash" href="/api-reference/bulk-transfers/delete">
    Remove a batch or item before processing starts.
  </Card>
...
1. **You submit a batch**, one of two ways:
   - A `POST` request with a `batchName` and a `transfers` array — one entry per payment you want to make. Interstellas validates the batch, writes a row for the batch and one row per transfer, and responds immediately with `201 Created`.
   - Or [upload an Excel spreadsheet](/api-reference/bulk-transfers/upload) instead — Interstellas parses it into the same batch/item structure, then verifies every recipient's account name in the background before you're expected to process it. This path has extra steps (poll a preview, fix anything that failed verification) that the JSON path doesn't need, since JSON submissions are trusted to already have correct recipient details.

   Either way, nothing has been paid out yet at this point — every item starts in `pending`. If you spot a mistake before processing, this is also your window to fix it — [edit](/api-reference/bulk-transfers/upload#edit-an-item) or [delete](/api-reference/bulk-transfers/delete) a single item, or delete the whole batch and start over.
2. **You start processing.** Batches don't begin executing on their own — call [Process a bulk transfer](/api-reference/bulk-transfers/process) with the `bulkTransferId` once you're ready. This queues every `pending` item for background execution and returns `202 Accepted` immediately. This is also the point of no return — batches and items can no longer be edited or deleted after this.
...
| `enquiry_complete` | Uploaded-via-Excel batches only — recipient name verification has finished for every row. See [Upload via Excel](/api-reference/bulk-transfers/upload). |
