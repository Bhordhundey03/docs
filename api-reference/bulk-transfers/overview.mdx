...
1. **You submit a batch**, one of two ways:
   ...
   Either way, nothing has been paid out yet at this point — every item starts in `pending`.
2. **You start processing.** Batches don't begin executing on their own — call [Process a bulk transfer](/api-reference/bulk-transfers/process) with the `bulkTransferId` once you're ready. This queues every `pending` item for background execution and returns `202 Accepted` immediately.
